...
let playbookCache: Map<string, { playbook: Playbook; mtime: number }> =
  new Map();
...
export function pruneBullets(
  bullets: Bullet[],
  threshold: number,
  staleDays: number = 14,
): { kept: Bullet[]; pruned: number } {
  const now = Date.now();
...
export function renderPlaybook(
  ...
  for (const [, arr] of grouped) {
    arr.sort(
      (a, b) =>
        b.helpful_count - b.harmful_count - (a.helpful_count - a.harmful_count),
    );
  }
...
export function applyImplicitFeedback(playbook: Playbook, trace: TraceEntry) {
...
export function loadRecentTraces(
  directory: string,
  maxEntries = 50,
): TraceEntry[] {
  const dir = path.join(aceDir(directory), TRACES_DIR);
    ...
    .map((t) => {
      const argsStr = t.args
        ? Object.entries(t.args)
            ...
            .map(
              ([k, v]) =>
                `${k}=${typeof v === "string" ? v.slice(0, 120) : JSON.stringify(v).slice(0, 120)}`,
            )
            .join(" ")
        : "";
...
export function applyReflection(
  ...
  if (playbook.bullets.length > playbook.config.max_bullets * 0.8) {
    const dedupResult = deduplicateBullets(
      playbook.bullets,
      playbook.config.dedup_threshold,
    );
    playbook.bullets = dedupResult.kept;
    merged = dedupResult.merged;

    const pruneResult = pruneBullets(
      playbook.bullets,
      playbook.config.prune_threshold,
      playbook.config.stale_bullet_ttl_days ?? 14,
    );
