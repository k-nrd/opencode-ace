...
export function createTools() {
  const tools = {
    ...
    ace_feedback: tool({
      description:
        "Provide feedback on a playbook bullet. Use this when you notice a bullet was helpful or harmful during your work.",
      args: {
        bullet_id: tool.schema
          .string()
          .describe("The ID of the bullet to rate"),
        rating: tool.schema
        ...
        note: tool.schema
          .string()
          .optional()
          .describe("Optional explanation of why this rating was given"),
      },
      async execute(args, context) {
    ...
    ace_reset: tool({
      description:
        "Reset the ACE playbook. Use 'soft' to prune low-scoring bullets, or 'hard' to delete everything.",
      args: {
        mode: tool.schema
          .enum(["soft", "hard"])
          .describe(
      ...
      async execute(args, context) {
        ...
        const { kept, pruned } = pruneBullets(
          playbook.bullets,
          playbook.config.prune_threshold,
        );
        const { kept: deduped, merged } = deduplicateBullets(
          kept,
