  ...
  function detectSessionTags(
    ...
    if (tool === "bash") {
      ...
      if (typeof cmd === "string") {
        ...
        if (/\b(python|pip|pytest|mypy|ruff)\b/.test(cmd))
          sessionTags.add("python");
        if (/\b(tsc|tsx|tsx|npm|pnpm|yarn|bun)\b/.test(cmd))
          sessionTags.add("typescript");
  ...
  return {
    ...
    config: async (input) => {
      ...
      input.command["ace-init"] = {
        template:
          "Run the ace_init tool to initialize an ACE playbook. $ARGUMENTS",
        description: "Initialize ACE playbook",
      };
      input.command["ace-reset"] = {
        template:
          'Run the ace_reset tool with mode "$1". Default to soft if not specified.',
        description: "Reset ACE playbook (soft/hard)",
      };
      input.command["ace-feedback"] = {
        template:
          'Run the ace_feedback tool with bullet_id "$1" and rating "$2". $ARGUMENTS',
    ...
    "tool.execute.after": async (input, output) => {
      ...
      const outcome =
        typeof output.output === "string" ? output.output.slice(0, 500) : null;
      const isRetry =
        ...
        try {
          for (let round = 0; round < maxRounds; round++) {
            const prompt = buildReflectionPrompt(
              traces,
              playbook,
              playbook.config.reflection_prompt,
              round,
            );
            ...
            try {
              const response = await client.session.prompt({
                ...
                body: {
                  system:
                    "You are the Reflector in an ACE (Agentic Context Engineering) loop. Respond with ONLY a JSON object, no markdown fences.",
              ...
              await client.session
                .delete({ path: { id: reflectSessionId } })
                .catch(() => {});
            }
