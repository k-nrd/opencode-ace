...
import {
  estimateTokens, extractFileExtension, detectErrorSignal,
  pruneBullets, renderPlaybook,
...
describe("extractFileExtension", () => {
  ...
  test("handles nested paths", () => {
    expect(extractFileExtension({ filePath: "packages/core/src/utils/helpers.ts" })).toBe("ts")
  })
  ...
  test("rejects very long extensions", () => {
    expect(extractFileExtension({ filePath: "file." + "a".repeat(20) })).toBeNull()
  })
})
...
describe("pruneBullets with stale TTL", () => {
  ...
  function makeBullet(daysAgo: number, helpful: number, harmful: number): Bullet {
    return {
      id: "test",
      content: "test bullet",
      category: "strategy",
...
describe("renderPlaybook with filters", () => {
  const basePlaybook: Playbook = {
    ...
    bullets: [
      { id: "a1", content: "Use cargo check for Rust", category: "strategy", helpful_count: 5, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["rust", "testing"] },
      { id: "b2", content: "Always check nulls", category: "pitfall", helpful_count: 2, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: [] },
      { id: "c3", content: "Python uses virtualenv", category: "domain", helpful_count: 3, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["python"] },
      { id: "d4", content: "Bad advice", category: "strategy", helpful_count: 0, harmful_count: 5, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["rust"] },
      { id: "e5", content: "Use tsconfig paths", category: "tool_use", helpful_count: 1, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["typescript"] },
    ],
    stats: { total_sessions: 0, total_reflections: 0, total_bullets_added: 0, total_bullets_pruned: 0, total_bullets_merged: 0 },
    config: {
      max_bullets: 200, dedup_threshold: 0.85, prune_threshold: -2,
      categories: ["strategy", "pitfall", "domain", "tool_use", "pattern"],
      reflection_prompt: null, curation_prompt: null, auto_inject: true,
      max_inject_tokens: 4000, min_score_to_inject: 1,
      max_reflection_rounds: 3, min_traces_for_reflection: 5,
      stale_bullet_ttl_days: 14, inject_categories: ["pitfall", "domain"], tag_matching: true,
    },
  }
  ...
  test("bullet order sorted by score within category", () => {
    const bullets = [
      { id: "low", content: "low score", category: "strategy", helpful_count: 1, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["ts"] },
      { id: "high", content: "high score", category: "strategy", helpful_count: 10, harmful_count: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), source_session: null, tags: ["ts"] },
    ]
    const p = { ...basePlaybook, bullets }
    const rendered = renderPlaybook(p, { tags: new Set(["ts"]), minScore: 0, alwaysCategories: [] })
