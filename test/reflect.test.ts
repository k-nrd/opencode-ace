  ...
  test("extracts text from SDK success response", () => {
    const response = {
      data: {
        info: { id: "msg_123" },
        parts: [
          { type: "text", text: '{"reasoning":"test","new_bullets":[],"updated_bullet_ids":[]}' },
        ],
      },
      error: undefined,
      request: {},
  ...
  test("throws on SDK error response (was silently producing empty reflection)", () => {
    const errorResponse = {
      error: {
        ...
        error: [{ origin: "string", code: "invalid_format", message: 'must start with "ses"' }],
        success: false,
      },
      request: {},
      response: {},
...
describe("full pipeline: SDK response → reflection result", () => {
  test("success path produces populated reasoning", () => {
    ...
    expect(reflection!.reasoning).toBe("Traces show multiple unhandled subprocess errors.")
